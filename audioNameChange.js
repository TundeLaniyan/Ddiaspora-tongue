...
const pathToOldFolders = [assetFolder, imageFolder, audioFolder];
...
for(const pathToOldFolder of pathToOldFolders) {
  for (const oldFile of readdirSync(pathToOldFolder)) {
      ...
      if (['.svg', '.jpg', '.png', '.m4a'].includes(extension)) {
          renameSync(join(pathToOldFolder, oldFile), join(pathToOldFolder, name.replace(/\s/g,'-').toLowerCase() + extension));
      }
  }
}
